...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $payload = [
        ...
        'id_espece'      => (int)($_POST['id_espece'] ?? 0) ?: null,
        'hauteur_totale' => $_POST['hauteur_totale'] !== '' ? (float)$_POST['hauteur_totale'] : null,
        'hauteur_tronc'  => $_POST['hauteur_tronc']  !== '' ? (float)$_POST['hauteur_tronc']  : null,
        'diametre_tronc' => $_POST['diametre_tronc'] !== '' ? (float)$_POST['diametre_tronc'] : null,
        ...
        'id_quartier'    => (int)($_POST['id_quartier']   ?? 0) ?: null,
        'id_situation'   => (int)($_POST['id_situation']  ?? 0) ?: null,
        // 'id_porte'       => (int)($_POST['id_porte']      ?? 0) ?: null,
        // 'id_pied'        => (int)($_POST['id_pied']       ?? 0) ?: null,
    ];
    $payload = array_filter($payload, fn($v) => $v !== null);
    $result  = createArbre($payload);
    if ($result) {
        $success = "Arbre ajouté (ID : " . ($result['id'] ?? '?') . ")";
    } else {
...
<head>
  ...
  <title>Ajouter un arbre — ArboData</title>
...
<div class="container">
  ...
  <?php if ($success): ?>
    <div class="alert alert-ok">✓ <?= htmlspecialchars($success) ?> — <a href="visualisation.php" style="color:inherit;font-weight:600;">Voir tous les arbres</a></div>
  <?php endif; ?>
  <?php if ($error): ?>
    ...
    <form method="POST" action="ajouter.php" novalidate>
      ...
      <div class="form-body">
        <div class="form-grid" style="margin-bottom:1rem;">
          <div class="field">
            <label>Y (EPSG:3949)</label>
            <input type="number" name="latitude" id="lat" step="0.01"
              placeholder="ex : 9197856.12"
              value="<?= htmlspecialchars($_POST['latitude'] ?? '') ?>">
          </div>
          <div class="field">
            <label>X (EPSG:3949)</label>
            <input type="number" name="longitude" id="lng" step="0.01"
              placeholder="ex : 1702456.44"
              value="<?= htmlspecialchars($_POST['longitude'] ?? '') ?>">
          </div>
        </div>
        <p style="font-size:.8rem; color:var(--muted); margin-bottom:.6rem;">Cliquez sur la carte (WGS84). Les champs sont remplis en X/Y convertis en EPSG:3949.</p>
        <div id="map-pick" style="height:300px;"></div>
      </div>

      <div class="form-footer">
        <a href="visualisation.php" class="btn btn-ghost">Annuler</a>
...
(function() {
  ...
  const map   = L.map('map-pick').setView([49.8489, 3.2870], 13);
